...
namespace AE_ClientNet
{
    ...
    public class ProtocolBufferToolWindow : EditorWindow
    {
        ...
        private int lauage;
        private string protocal_root_path;
        ...
        {
            string url = EditorPrefs.GetString( "ProtocolConfig", string.Empty );
            if ( !string.IsNullOrEmpty( url ) )
            {
                data = AssetDatabase.LoadAssetAtPath<ProtocolGenrateData>( url );
                if ( data != null )
                {
                    InitProtocolGenrateData( );
                }
            }
        ...
        private void InitProtocolGenrateData( )
        {
            protocal_root_path = Directory.GetCurrentDirectory( ) + "\\Assets/Protocal";
            data.protoFilePath = protocal_root_path + "/proto";
            data.outputFilePath = protocal_root_path + "/csharp";
            data.protoExepath = Directory.GetCurrentDirectory( ) + "\\Tools\\protoc-3.5.1-win32\\bin\\protoc.exe";
            data.messageXMLFilePath = protocal_root_path + "/proto/Messages.XML";
        }
        ...
        private void OnGUI( )
        {
            ...
            {
                data = temp_data;
                string url = AssetDatabase.GetAssetPath( data );
                EditorPrefs.SetString( "ProtocolConfig", url );
                InitProtocolGenrateData( );
            }
